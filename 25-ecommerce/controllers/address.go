...
func AddAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		group := bson.D{{Key: "$group", Value: bson.D{primitive.E{Key: "_id", Value: "$address_id"}, {Key: "count", Value: bson.D{primitive.E{Key: "$sum", Value: "$"}}}}}}
		...
		if size < 2 {
			...
			update := bson.D{{Key: "$push", Value: bson.D{primitive.E{Key: "_id", Value: address}}}}
...
func DeleteAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		filter := bson.D{primitive.E{Key: "_id", Value: usert_id}}
		update := bson.D{{Key: "$set", Value: bson.D{primitive.E{Key: "address", Value: addresses}}}} //set the value to an empty slice in memory
		_, err = UserCollection.UpdateOne(ctx, filter, update)
		if err != nil {
